...
import com.oumen.circle.list.CircleHttpController1.CircleDataWrapper;
import com.oumen.circle.list.CircleHttpController1.ObtainType;
...
public class CircleController1 implements Handler.Callback, View.OnClickListener, OnScrollListener {
	protected final AdapterImpl adapter = new AdapterImpl();

	protected final Handler handler = new Handler(this);
	protected final CircleHttpController1 httpController = new CircleHttpController1(handler);

	protected CircleListFragment1 host;
	...
	CircleController1(CircleListFragment1 host) {
		this.host = host;
...
//						changeBackground();
			...
			if (requestCode == CircleListFragment1.REQUEST_SHARE) {// 发表成功了偶们圈的内容
	...
	public class CircleListAdapter extends HSZListViewAdapter<CircleUserMsg> implements View.OnClickListener {
		...
		synchronized public View getView(int position, View convertView, ViewGroup parent) {
			CircleItem1 item = null;
			if (convertView == null) {
				CircleItemData itemData = new CircleItemData();
				item = new CircleItem1(parent.getContext());
...
//				host.lstView.setLoadingViewVisibility(View.VISIBLE);
	...
	public void onClick(View v) {
		...
		switch (id) {
			...
			case R.id.banner_message:
				...
				host.getActivity().startActivityForResult(intent, CircleListFragment1.REQURST_NOTICE_MESSAGE);
				break;
		}
	}
	...
	public boolean handleMessage(Message msg) {
		switch (msg.what) {
			case CircleHttpController1.HANDLER_OBTAIN_LIST:
				isLoading = false;
...
//							host.lstView.setLoadingViewVisibility(View.GONE);
			...
			case CircleHttpController1.HANDLER_DELETE_COMMENT:
				synchronized (adapter) {
			...
			case CircleHttpController1.HANDLER_DELETE_CONTENT:
				synchronized (adapter) {
					if (msg.arg1 == 0 && msg.obj == null) {
						adapter.notifyDataSetChanged();
					}
			...
			case CircleHttpController1.HANDLER_SEND_CONTENT:
				synchronized (adapter) {
					if (msg.arg1 == 0 && msg.obj == null) {
						adapter.notifyDataSetChanged();
					}
			...
			case CircleHttpController1.HANDLER_SEND_COMMENT:
				synchronized (adapter) {
					if (msg.arg1 == 0 && msg.obj == null) {
						adapter.notifyDataSetChanged();
					}
			...
			case CircleHttpController1.HANDLER_ENJOY:
				synchronized (adapter) {
					if (msg.arg1 == 0 && msg.obj == null) {
						adapter.notifyDataSetChanged();
					}
			...
			case CircleHttpController1.HANDLER_UNENJOY:
				synchronized (adapter) {
					if (msg.arg1 == 0 && msg.obj == null) {
						adapter.notifyDataSetChanged();
					}
